...
class KGTrainDataset(data.Dataset):
    ...
    def sample_negs(self):
        ...
        item_num = configs['data']['item_num']
        neg_candidates = np.random.randint(item_num, size=len(self.train_cf_pairs)*10)
        idx = 0
        for i in tqdm(range(len(self.train_cf_pairs))):
            u = self.train_cf_pairs[i][0]
            for _ in range(10):     # 最多10次负采样
                neg_i = neg_candidates[idx]
                idx += 1
...
class KGTripletDataset(data.Dataset):
    def __init__(self, kg_triplets, kg_dict) -> None:
        ...
        self.negs = self._neg_sample()

    def __len__(self):
        return len(self.kg_triplets)

    def _neg_sample(self):
        negs = np.zeros(len(self.kg_triplets))
        neg_candidates = np.random.randint(configs['data']['entity_num'], size=len(self.kg_triplets)*10)
        idx = 0
                
        num_fail = 0
        for i in tqdm(range(len(self.kg_triplets)), desc="neg sampling"):
            h, r, t = self.kg_triplets[i]
            for _ in range(5):
                neg_t = neg_candidates[idx]
                idx += 1
    ...
    def _neg_sample(self):
        negs = np.zeros(len(self.kg_triplets))
        neg_candidates = np.random.randint(configs['data']['entity_num'], size=len(self.kg_triplets)*10)
        negs = neg_candidates[:len(self.kg_triplets)]
        return negs
    ...
    def _neg_sample_kg(self, h, r):
        neg_t = self.neg_cand[self.net_idx]
        self.net_idx += 1
        if self.net_idx >= len(self.neg_cand):
            self.net_idx = 0
            self.neg_cand = np.random.randint(configs['data']['entity_num'], size=10000)
        if (r, neg_t) not in self.kg_dict[h]:
            return neg_t
        else:
           return 0

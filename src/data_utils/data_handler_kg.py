...
class DataHandlerKG:
    def __init__(self) -> None:
        ...
        elif configs['data']['name'] == 'Amazon_Books':
            predir = './datasets/kg/Amazon_Books_kg/'
        elif configs['data']['name'] == 'lfm-small':
            predir = './datasets/kg/lfm-small_kg/'
        elif configs['data']['name'] == 'ml-1m':
            predir = './datasets/kg/ml-1m_kg/'
        elif configs['data']['name'] == 'ml-100k':
            predir = './datasets/kg/ml-100k_kg/'
        else:
            raise Exception("Please provide the correct dataset name through --dataset.")
    ...
    def _build_graphs(self, train_data, triplets):
        ...
        for h_id, r_id, t_id in tqdm(triplets, ascii=True):
            ...
            # NOTE: 
            if len(kg_dict[h_id]) > 1000:
                kg_dict[h_id] = [(r_id, t_id)]
    ...
    def load_data(self):
        ...
        # valid_data = KGTestDataset(self.test_user_dict, self.train_user_dict)
        # self.valid_dataloader = data.DataLoader(valid_data, batch_size=configs['test']['batch_size'], shuffle=False, num_workers=0)
        ...
        if 'train_trans' in configs['model'] and configs['model']['train_trans']:
            ...
            self.triplet_dataloader = data.DataLoader(triplet_data, batch_size=configs['train']['kg_batch_size'], shuffle=False, num_workers=32)
